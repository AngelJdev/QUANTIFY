...
import { FiStar, FiLock, FiRefreshCw, FiTarget } from 'react-icons/fi';
import {
    GiRocket, GiMedal, GiBrain, GiMoon,
    GiWaterDrop, GiDutchBike, GiRun, GiWeightLiftingUp
} from 'react-icons/gi';
import api from '../services/api';

// ─── ICON MAP (react-icons/gi — MIT License) ──────────────────────────────
const ACHIEVEMENT_ICONS = {
    'chispa':     { Icon: GiMedal,           color: '#f97316' },
    ...
    'arquitecto': { Icon: GiMedal,            color: '#fbbf24' },
    'alquimista': { Icon: GiMedal,            color: '#a3e635' },
    'estelar':    { Icon: GiMedal,            color: '#fb923c' },
...
const getRequirement = (titulo = '') => {
    const t = titulo.toLowerCase();
    const key = Object.keys(ACHIEVEMENT_REQUIREMENTS).find(k => t.includes(k));
    return key ? ACHIEVEMENT_REQUIREMENTS[key] : null;
};

const getIcon = (titulo = '') => {
    const t = titulo.toLowerCase();
    const key = Object.keys(ACHIEVEMENT_ICONS).find(k => t.includes(k));
    return ACHIEVEMENT_ICONS[key] || ACHIEVEMENT_ICONS['default'];
};

const getRarityStyle = (titulo = '') => {
    const t = titulo.toLowerCase();
...
// ─── MOCK DATA ─────────────────────────────────────────────────────────────
const mockLogros = [
    { id: 1, titulo: 'Chispa Inicial', descripcion: 'Primeros 3 días de racha. ¡El motor ha arrancado!', mes_logro: 'Abril 2026', icono_url: 'award' },
    { id: 2, titulo: 'Racha Estelar 7', descripcion: 'Has mantenido el foco durante una semana completa.', mes_logro: 'Abril 2026', icono_url: 'star' },
    { id: 3, titulo: 'Alquimista de Datos', descripcion: 'Has procesado más de 30 registros bio-sincrónicos.', mes_logro: 'Abril 2026', icono_url: 'tool' },
    { id: 4, titulo: 'Arquitecto de Hábitos', descripcion: 'Has configurado tu primer sistema de ingeniería personal.', mes_logro: 'Abril 2026', icono_url: 'layout' },
    { id: 5, titulo: 'Imparable', descripcion: 'Has alcanzado los 66 días de racha. ¡Tus hábitos son ahora parte de tu ADN!', mes_logro: 'Abril 2026', icono_url: 'zap' },
                ...
                {achievements.length === 0 ? (
                    ...
                    <motion.div key="grid" variants={container} initial="hidden" animate="show"
                        ...
                        {achievements.map((achievement) => {
                            const rarity = getRarityStyle(achievement.titulo);
                            const { Icon, color } = getIcon(achievement.titulo);
                            const req = getRequirement(achievement.titulo);
                            return (
                                <motion.div
                                    key={achievement.id}
                                ...
                                >
                                    ...
                                    {/* Requirement pill — always visible */}
                                    {req && (
